...
def home():
    if request.method == 'POST':
        if not request.form['principleAmount']:
            ...
            stocksInput = [stock_one, stock_two, stock_three, stock_four]
            print(stocksInput)
            data = quandl.get_table('WIKI/PRICES', ticker=stocksInput,
                                    qopts={'columns': [
            ...
            cr2020_A = int(table.at['2018-01-05', stocksInput[0]])
            cr2020_B = int(table.at['2018-01-05', stocksInput[1]])
            cr2020_C = int(table.at['2018-01-05', stocksInput[2]])
            cr2020_D = int(table.at['2018-01-05', stocksInput[3]])
            ...
            display_simulated_ef_with_random(
                mean_returns, cov_matrix, num_portfolios, risk_free_rate, stocksInput)
            principleAmount = int(principleAmount)

            princealloc_min_A = int((alloc_min_A/100) * principleAmount)
            princealloc_min_B = int((alloc_min_B/100) * principleAmount)
            princealloc_min_C = int((alloc_min_C/100) * principleAmount)
            princealloc_min_D = int((alloc_min_D/100) * principleAmount)
            ...
            princealloc_bench_A = int(0.25 * principleAmount)
            princealloc_bench_B = int(0.25 * principleAmount)
            princealloc_bench_C = int(0.25 * principleAmount)
            princealloc_bench_D = int(0.25 * principleAmount)

            global sharesMin
            global sharesSharpe
            global sharesBench

            shares_min_A = princealloc_min_A // cr2020_A
            shares_min_B = princealloc_min_B // cr2020_B
            shares_min_C = princealloc_min_C // cr2020_C
            shares_min_D = princealloc_min_D // cr2020_D
            sharesMin = [shares_min_A, shares_min_B,
                         shares_min_C, shares_min_D]
            # Calculating the number of shares obtainable per share based on Sharpe Ratio

            shares_sr_A = princealloc_sr_A // cr2020_A
            shares_sr_B = princealloc_sr_B // cr2020_B
            shares_sr_C = princealloc_sr_C // cr2020_C
            shares_sr_D = princealloc_sr_D // cr2020_D
            sharesSharpe = [shares_sr_A, shares_sr_B, shares_sr_C, shares_sr_D]

            # Calculating the number of shares obtainable per share for the Benchmark

            shares_bench_A = princealloc_bench_A // cr2020_A
            shares_bench_B = princealloc_bench_B // cr2020_B
            shares_bench_C = princealloc_bench_C // cr2020_C
            shares_bench_D = princealloc_bench_D // cr2020_D
            sharesBench = [shares_bench_A,
                           shares_bench_B, shares_bench_C, shares_bench_D]
            return render_template("index.html", stocksInput=stocksInput, stocks=stocksList, ptype="submitted", sharesMin=sharesMin, sharesSharpe=sharesSharpe, sharesBench=sharesBench, )
    else:
...
def charts():
    return render_template('charts.html', sharesMin=sharesMin, sharesSharpe=sharesSharpe, sharesBench=sharesBench,)
